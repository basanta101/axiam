    ...
    return (
        <>
            {product[SELECT_KEY] ? <div className="flex-cc h-100-p flex-js">
                ...
                <Button classes='bg-none' variant={BUTTON_VARIANT.TRANSPARENT} onClick={() => onRemoveProduct(product)}>
                    <Button.Icon icon={ICON_TYPE.CANCEL} />
                    <Button.Text text='Remove' classes='btn-text text-black' />
                </Button>
            </div> :
                <div className="default-view-wrap bg-anti-flash-white b-r4 p-16">
...
const GridComponent = ({ numberOfItems = 4, onRemoveProduct = f => f, selectedProducts = [] }) => {
    const generateGridItems = () => {
        ...
        for (let i = 0; i < numberOfItems; i++) {
            items.push(<div key={i} className='h-180 b-r12 flex-c box-shadow border-culture-white'><Product product={selectedProducts[i]} onRemoveProduct={onRemoveProduct} /></div>);
        }
        return items;
    };
